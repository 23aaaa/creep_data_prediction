...
model.fit(X_train.reshape(X_train.shape[0], X_train.shape[1], 1), y_train, epochs=1500, batch_size=32, verbose=1)
...
ga_instance = pygad.GA(num_generations=50,
                       ...
                       sol_per_pop=20,
                       num_genes=4,
                       gene_space=[{'low': 0.0001, 'high': 0.1},  # 学习率
                                   {'low': 16, 'high': 128},      # 卷积核数量
                                   {'low': 1, 'high': 5},         # 卷积核大小
                                   {'low': 800, 'high': 2500}],   # 训练轮数
...
try:
    ...
    print("Interrupted by user. Showing the best solution so far.")
    best_solution, best_solution_fitness, best_solution_idx = ga_instance.best_solution()
    print("Best solution: {0}, fitness: {1}".format(best_solution, best_solution_fitness))
